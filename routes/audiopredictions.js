...
router.post('/audio_to_emotion', async (req, res) => {
    try {
        const { meet_id, host_id, studentPID, audio_message, time_stamp } = req.body; // Extract studentPID and message from request body
        if (!meet_id || !host_id || !studentPID || !audio_message) throw new Error("Student PID and message are required.");
